...
exports.getAllProducts= catchAsync(async (req,res,next)=>{
    let products= await ProductModel.find().populate('categories.main categories.sub','-name')
    res.status(200).json({ message: 'All products', products });
}) 

exports.getProductById= catchAsync(async (req,res,next)=>{
    const product = await ProductModel.findById(req.params.id).populate('categories.main categories.sub','-name');
