...
exports.toggleWishlist = catchAsync(async (req, res, next) => {
  ...
  if (index > -1) {
    ...
    await user.save();
    return res.status(200).json({ message: 'Removed from wishlist', wishlist: user.wishlist, inWishlist: false });
  } else {
    user.wishlist.push(productId);
    await user.save();
    return res.status(200).json({ message: 'Added to wishlist', wishlist: user.wishlist, inWishlist: true });
  }
});
